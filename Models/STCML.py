...
class STCML(nn.Module):
    def __init__(self,hps):
        ...
        self.dec = nn.GRUCell(self.hps['n_neighbors'],self.hps['hid2'])
        self.fc = nn.Linear(self.hps['hid2'],1)

    def globalAttn(self,s,x,x_int,getAttn=False):
    ...
    def forward(self,x,getGA=False,getTA=False):
        ...
        for t in range(-self.hps['l1'],0,1):
            x_tilde = self.globalAttn(s,x_m,x[1][:,t,:]) # (128,num_mobile)
            s = self.enc2(x_tilde,s)
        
        
        out = self.dec(x_tilde,s)
